...
#define SERVER_STRING "Server: Team4\r\n"
...
void serve_header(int client, const char * filename);
...
void serve_header(int client, const char * filename)
{
	printf("\nInside serve_header:%s", filename);
	FILE *resource = NULL;
	int numchars = 1;
	char buf[1024];
...
void http_not_supported(int client)
{
	char buf[1024];
	...
	sprintf(buf, "\r\n");
	send(client, buf, sizeof(buf), 0);
	sprintf(buf, "<P>Your browser sent a bad HTTP Version, ");
	send(client, buf, sizeof(buf), 0);
	sprintf(buf, "such as HTTP/1.0.\r\n");
	send(client, buf, sizeof(buf), 0);
}
...
void accept_request(int client)
{
	...
	while (ISspace(buf[j]) && (j < sizeof(buf)))
		j++;
	while (!ISspace(buf[j]) && (i < sizeof(http_version) - 1) && (j < sizeof(buf)))
	{
	...
	printf("\nHTTP version:%s", http_version);  /*****505 error code checking!*******/

	if(strcmp(http_version, "HTTP/1.0")==0)
	{
		printf("\nHTTP version not supported!");
		http_not_supported(client);
		return;
	}
	//printf("\nURL:%s", url);
	...
	if(strcasecmp(method, "HEAD")==0)      /**************HEAD method implementation*********/
	{
		printf("\nInside serve_header");	
		serve_header(client, path);
		return;
	}
...
void request_too_large(int client)
{
	char buf[1024];

	sprintf(buf, "HTTP/1.1 413 Request Entity Too Large\r\n");
	send(client, buf, sizeof(buf), 0);
	sprintf(buf, "Content-type: text/html\r\n");
	send(client, buf, sizeof(buf), 0);
	sprintf(buf, "\r\n");
	send(client, buf, sizeof(buf), 0);
	sprintf(buf, "<P>Your browser sent too large a request, ");
	send(client, buf, sizeof(buf), 0);
	sprintf(buf, "such as a POST with a huge file for upload!\r\n");
	send(client, buf, sizeof(buf), 0);
}
	...
	else    /* POST */
	{
		...
		while ((numchars > 0) && strcmp("\n", buf))
		{
			...
			if (strncasecmp(buf, "Content-Length:", 15) == 0) {
				...
				if(content_length>5000)    /*********413 error code handled here!**********/
				{
					printf("\nContent-length greater than limit! Error 413 will now ne handled");
...
int main(void)
{
	...
	serv_sock=socket(AF_INET6, SOCK_STREAM, IPPROTO_IP);

	printf("\nServer Socket created!:%d", serv_sock);

	setsockopt(serv_sock, SOL_SOCKET, SO_REUSEADDR,(char*)&on,sizeof(on) );
